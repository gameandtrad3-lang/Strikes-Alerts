...
const STRIKES = [
  ...
  },
  {
    ...
    date: "Results: Mar 23 â Strike ~Apr 2",
...
const CONTRACTS = [
  {
    ...
    pay_hr: "$80â130/hr", pay_wk: "$4,800â7,800/wk (est.)",
    type: "STRIKE", specialty: ["ICU", "L&D", "ER"],
    start: "Imminent â Corewell",
    agency: "Fastaff Â· Aya Â· USNursing",
    hot: true,
    note: "Pre-position NOW before 10-day notice drops and slots vanish",
  },
  {
    id: 2,
    title: "Strike RN â All Specialties",
    location: "Grand Blanc, MI",
    pay_hr: "$90â120/hr", pay_wk: "$5,400â7,200/wk",
    type: "STRIKE", specialty: ["Med-Surg", "ICU", "ER"],
    start: "Active now",
    agency: "USNursing Â· Cross Country",
    hot: false,
    note: "Henry Ford Genesys ~Day 200. Volume tapering â act fast",
  },
  {
    id: 3,
    title: "Crisis RN â Behavioral Health",
    location: "Northern California",
    pay_hr: "$95â115/hr", pay_wk: "$5,700â6,900/wk",
    type: "CRISIS", specialty: ["Psych", "Behavioral Health"],
    start: "Now",
    agency: "Aya Â· Vivian Â· Fastaff",
    hot: false,
  ...
  },
  {
    ...
    pay_hr: "$70â95/hr", pay_wk: "$4,200â5,700/wk (est.)",
    type: "PRE-POSITION", specialty: ["Med-Surg", "ER", "ICU"],
    start: "~Apr 2 if strike authorized",
    agency: "Fastaff Â· White Glove",
    hot: true,
    note: "Small hospital (160 RNs). Fast fill. Register before Mar 23 vote results.",
  },
  {
    id: 5,
    title: "Strike RN â NYC Benchmark (Closed)",
    location: "New York City, NY",
    pay_hr: "Up to $161/hr (peak)", pay_wk: "$10,500â$12,800/wk",
...
const TICKER_ITEMS = [
  "ð´ COREWELL HEALTH EAST MI â 10,000 RNs authorize strike â 10-day notice IMMINENT",
  "ð  BRATTLEBORO MEMORIAL VT â Strike vote results due MON MAR 23",
  "â KAISER UNAC/UHCP â Contracts RATIFIED Mar 20 â 31,000 workers return",
  "â NYC NYSNA â Strike ended Feb 21 after 41 days â all contracts ratified",
  "ð¡ HENRY FORD GENESYS MI â Day ~200 of strike â 'optimistic' joint statement",
  "ðµ NUHW KAISER MENTAL HEALTH CA â Active strike â 23K CNA sympathy Mar 18",
  "ð 2026 BENCHMARK: Strike RNs earned up to $161/hr Â· $12,800/wk NYC window",
  "â¡ MICHIGAN = #1 DEPLOYMENT ZONE â Corewell + Genesys dual flashpoint",
];
...
const INSIGHTS = [
  { icon: "â¡", title: "Michigan is the #1 Deployment Zone", body: "Corewell Health East (10K RNs, 9 hospitals) has a 90% strike authorization vote and no final offer. Henry Ford Genesys is 200 days in. Pre-position credentials with Fastaff, Aya, and USNursing immediately.", color: "var(--red)" },
  { icon: "ð°", title: "2026 Crisis Pay Benchmarks Reset Upward", body: "NYC strike saw rates up to $161/hr and $12,800/week. Kaiser crisis hit $9K/week. These numbers anchor expectations for every future deployment.", color: "var(--green)" },
  { icon: "ð§ ", title: "Psych/Behavioral Health: Hidden High-Pay Niche", body: "25% surge in psych contract offers in 2026. The NUHW Kaiser mental health dispute signals sustained demand. If you hold a psych cert, this market is underserved and growing fast.", color: "var(--accent)" },
  { icon: "ð¤", title: "AI Clauses Are Now Standard Bargaining Demands", body: "Every major 2026 contract now includes language blocking AI from replacing clinical judgment. This structural conflict will drive more disputes as hospital AI adoption accelerates.", color: "var(--yellow)" },
  { icon: "ð", title: "First Contracts = Highest Escalation Risk", body: "Corewell and BMC South are first-contract disputes â historically the most contentious type. These escalate faster and resolve slower than renewals.", color: "var(--orange)" },
];

// ââ TRAVEL CONTRACTS DATA âââââââââââââââââââââââââââââââââââââââââââââââââââââ
...
function Ticker() {
  const full = [...TICKER_ITEMS, ...TICKER_ITEMS].join("     Â·     ");
  return (
    ...
    <div onClick={() => onClick(s)} style={{ background: selected ? "var(--panel2)" : "var(--surface)", border: `1px solid ${selected ? statusColor : "var(--border)"}`, borderLeft: `6px solid ${statusColor}`, borderRadius: 8, padding: "16px 18px", cursor: "pointer", animation: `fadeUp 0.45s ease both`, animationDelay: `${idx * 60}ms`, transition: "all 0.15s", boxShadow: selected ? `0 2px 12px ${statusColor}22` : "0 1px 3px rgba(0,0,0,0.04)" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 12, marginBottom: 10 }}>
        <div>
          ...
          <div style={{ fontFamily: "var(--font-mono)", fontSize: 13, color: "var(--text2)" }}>{s.city}, {s.state} Â· {s.union}</div>
        </div>
...
function ContractCard({ c, idx }) {
  ...
  return (
    <div style={{ background: "var(--panel)", border: `1px solid ${c.hot ? "var(--red)" : "var(--border)"}`, borderRadius: 6, padding: "16px 18px", animation: `fadeUp 0.45s ease both`, animationDelay: `${idx * 70}ms`, position: "relative", overflow: "hidden", opacity: c.type === "CLOSED" ? 0.55 : 1 }}>
      {c.hot && <div style={{ position: "absolute", top: 0, right: 0, background: "var(--red)", color: "#fff", fontFamily: "var(--font-mono)", fontSize: 9, fontWeight: 700, letterSpacing: 1, padding: "3px 10px", borderBottomLeftRadius: 6 }}>â¡ ACT NOW</div>}
...
function PricingPage() {
  ...
  return (
    <div style={{ maxWidth: 800, margin: "0 auto" }}>
      <SectionHeader accent="var(--accent)">ð³ Plans & Pricing</SectionHeader>
      ...
      <div style={{ background: "var(--panel)", border: "1px solid var(--border)", borderRadius: 8, padding: "20px 24px", marginBottom: 28, textAlign: "center" }}>
        ...
        <div style={{ fontFamily: "var(--font-sans)", fontSize: 14, color: "var(--text2)" }}>
          Strike nurses earn $5,000â$12,800/week. At $14.99/mo, your subscription pays for itself in the first 30 minutes on assignment.
        </div>
      </div>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 10, marginBottom: 24 }}>
                {plan.features.map(f => (
                  <div key={f} style={{ display: "flex", alignItems: "flex-start", gap: 8 }}>
                    <span style={{ color: "var(--green)", fontFamily: "var(--font-mono)", fontSize: 12, flexShrink: 0, marginTop: 1 }}>â</span>
      ...
      <div style={{ background: "var(--panel)", border: "1px solid var(--border)", borderRadius: 8, padding: "20px 24px" }}>
        ...
        {[
          { q: "How often is the data updated?", a: "Intel is scanned and refreshed every 6â12 hours, and on-demand when major notices drop." },
        ...
        <div style={{ fontFamily: "var(--font-mono)", fontSize: 10, color: "var(--muted)" }}>Payments powered by Stripe Â· SSL secured Â· Cancel anytime</div>
      </div>
    </div>
  );
}

// ââ TRAVEL CONTRACTS PAGE âââââââââââââââââââââââââââââââââââââââââââââââââââââ
      ...
      <div style={{ background: D.surface, borderBottom: `1px solid ${D.border}`, padding: "16px 24px" }}>
        <div style={{ maxWidth: 1100, margin: "0 auto" }}>
          ...
          <div style={{ fontSize: 22, fontWeight: 800, color: D.text, marginBottom: 4 }}>
            ð¥ Travel Nurse Contract Assignments
          </div>
          <div style={{ fontSize: 13, color: D.text2 }}>
            Latest travel nurse openings by state, specialty & hospital â updated daily
          </div>
        </div>
      </div>
...
export default function App() {
  ...
  const TABS = [
    ...
    { id: "outlook",   label: "2â4 Wk Outlook",   count: null },
    { id: "insights",  label: "ð° News",             count: null },
    { id: "pricing",   label: "ð³ Pricing",        count: null },
  ];
    ...
    <>
      ...
      <div style={{ minHeight: "100vh", display: "flex", flexDirection: "column" }}>
        ...
        <header style={{ background: "var(--surface)", borderBottom: "1px solid var(--border)", padding: "0 24px", height: 64, display: "flex", alignItems: "center", justifyContent: "space-between", boxShadow: "0 1px 3px rgba(0,0,0,0.06)" }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 18 }}>
            ...
            <div style={{ fontFamily: "var(--font-mono)", fontSize: 10, color: "var(--muted)" }}>
              {now.toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" })} Â· {now.toLocaleTimeString("en-US", { hour: "2-digit", minute: "2-digit" })}
            </div>
        ...
        <div style={{ display: "flex", gap: 10, padding: "14px 24px", background: "var(--surface)", borderBottom: "1px solid var(--border)", overflowX: "auto" }}>
          ...
          <StatCard label="AUTHORIZED" value="2" sub="Corewell Â· BMC South" color="var(--red)" delay={60} />
          <StatCard label="VOTES PENDING" value="1" sub="Brattleboro â Mar 23" color="var(--orange)" delay={120} />
        ...
        <main style={{ flex: 1, padding: "24px", maxWidth: 1100, width: "100%", margin: "0 auto" }}>
          ...
          {tab === "strikes" && (
            <div>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 18, flexWrap: "wrap", gap: 10 }}>
                <SectionHeader accent="var(--red)">ð¨ Active Labor Disputes & Strike Notices</SectionHeader>
              ...
              <div style={{ marginTop: 14, padding: "10px 14px", background: "var(--panel)", border: "1px solid var(--border)", borderRadius: 6, fontFamily: "var(--font-mono)", fontSize: 10, color: "var(--muted)" }}>
                â¸ Tap any card to expand Â· Sources: NNU, NYSNA, Teamsters, CNA, Nurse.org, local news
              </div>
            </div>
          )}
          ...
          {tab === "contracts" && (
            <div>
              <SectionHeader accent="var(--green)">ð° High-Pay Strike & Crisis RN Contracts</SectionHeader>
              <div style={{ background: "var(--red-dim)", border: "1px solid rgba(255,77,109,0.3)", borderRadius: 6, padding: "10px 14px", marginBottom: 18, fontFamily: "var(--font-mono)", fontSize: 11, color: "var(--red)", lineHeight: 1.6 }}>
                â¡ ALERT: Corewell Health East (10K RNs, SE Michigan) â pre-position NOW before 10-day notice drops.
              </div>
          ...
          {tab === "outlook" && (
            <div>
              <SectionHeader accent="var(--yellow)">ð 2â4 Week Strike Probability Outlook</SectionHeader>
              ...
              <div style={{ marginTop: 14, padding: "12px 16px", background: "var(--yellow-dim)", border: "1px solid rgba(255,214,10,0.25)", borderRadius: 6, fontFamily: "var(--font-mono)", fontSize: 11, color: "var(--yellow)", lineHeight: 1.8 }}>
                â  NEXT TRIGGERS: Brattleboro vote results Mon Mar 23 Â· Corewell 10-day notice (watch daily) Â· Henry Ford Genesys resolution TBD
              </div>
            </div>
          )}
          ...
          {tab === "insights" && (
            <div>
              <SectionHeader accent="var(--accent)">ð° Latest Strike News</SectionHeader>
              <div style={{ background: "var(--accent-dim)", border: "1px solid rgba(0,119,170,0.2)", borderRadius: 8, padding: "12px 16px", marginBottom: 18, fontFamily: "var(--font-sans)", fontSize: 13, color: "var(--accent)" }}>
                ð News is updated daily at 6 AM ET â sourced from union announcements, local news, and healthcare publications.
              </div>
        ...
        <footer style={{ background: "var(--surface)", borderTop: "1px solid var(--border)", padding: "10px 24px", display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 6 }}>
          <div style={{ fontFamily: "var(--font-mono)", fontSize: 10, color: "var(--muted)" }}>StrikeIntel Â· Healthcare Labor Intelligence Â· Not legal or financial advice</div>
          <div style={{ fontFamily: "var(--font-mono)", fontSize: 10, color: "var(--muted)" }}>Scan cycle: 6â12hr Â· Powered by Claude AI</div>
        </footer>
      </div>
    </>
  );
